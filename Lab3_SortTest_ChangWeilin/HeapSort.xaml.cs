...
*@About: This program is the window of the HeapSort Algorithms Test Application With C# WPF.
*/
...
{
    //堆排序窗口类
    public partial class HeapSort : Window
    {
        ...
        private void Start_Click(object sender, RoutedEventArgs e)
        {
            ...
            try
            {
                ...
                //数据全部就绪后（也就是读入数据完毕后），开始对构造堆及堆排序的过程计时
                _data = HeapSortAlgorithm(_data, out TimeSpan heapConstructionTime, out TimeSpan sortingTime);
                txtInformation.Text = $"构建堆时间为： {heapConstructionTime.TotalMilliseconds} 毫秒; 堆排序时间为： {sortingTime.TotalMilliseconds} 毫秒";
                ...
                double sortingTimeMs = sortingTime.TotalMilliseconds + heapConstructionTime.TotalMilliseconds;

                MessageBox.Show($"排序成功！总耗时为：{sortingTime.TotalMilliseconds + heapConstructionTime.TotalMilliseconds} 毫秒。排序结果已保存至：{outputPath}", "排序已结束", MessageBoxButton.OK, MessageBoxImage.Information);
                _filePath = null;
                ttime = sortingTime.TotalMilliseconds + heapConstructionTime.TotalMilliseconds;
        ...
        //堆排序的核心算法，输入一个double类型数组，传入三个参数：数据数组，构造堆的时间，堆排序的时间
        //输出堆构造时间和堆排序时间和排好序的数据
        private double[] HeapSortAlgorithm(double[] data, out TimeSpan heapConstructionTime, out TimeSpan sortingTime)
        {
            int n = data.Length;
            ...
            stopwatch.Stop();
            heapConstructionTime = stopwatch.Elapsed;

            stopwatch.Restart();
            //遍历堆中每一个元素
            for (int i = n - 1; i >= 0; i--)
            {
                //进行堆排序：交换当前的根节点（即最大值）与最后一个节点
                (data[i], data[0]) = (data[0], data[i]);

                //递归调用LessHeap函数，实现每次排序后堆的规模都能逐次减少
                LessHeap(data, i, 0);
            }
            stopwatch.Stop();
        ...
        //构建最大堆的函数
        //对所有受影响的子树进行递归，不断减小堆的规模
        private void LessHeap(double[] data, int n, int i)
        {
            int largest = i;
            int left = 2 * i + 1;
            int right = 2 * i + 2;
